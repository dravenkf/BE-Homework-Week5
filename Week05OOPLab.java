...
public class Week05OOPLab {
	...
	public static void main(String[] args) {
		...
		Card c1 = new Card("Ace", "Spades", 14);
		...
		int playerCount = 3;
		Map <String, List<Card>> game = gameMaker(playerCount);
	...
	// Method 5:
	public static Map<String, List<Card>> gameMaker (int count) {
		...
		for (int i = 1; i <= count; i++) {
			...
			String player = "Player " + i;
			gameMap.put(player, playerCards);
		}
		
		for (int i = 0; i < 52/count; i++) {
			for (int j = 1; j <= count; j++) {
				String player = "Player " + j;
				List<Card> playerCards = gameMap.get(player);
				playerCards.add(deck.draw());
				gameMap.replace(player, playerCards);
			}
		}
		System.out.println(52%count + " card(s) are left in the deck.");
		System.out.println();
		return gameMap;
